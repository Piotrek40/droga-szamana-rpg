...
class NPC:
    ...
    def modify_emotion(self, emotion: EmotionalState, intensity: float):
        ...
        # Defensywne sprawdzenie - upewnij się że klucz istnieje
        if emotion not in self.emotional_states:
            self.emotional_states[emotion] = 0.0

        self.emotional_states[emotion] = min(1.0, self.emotional_states[emotion] + intensity)

        # Zmniejsz neutralność
        if EmotionalState.NEUTRAL not in self.emotional_states:
            self.emotional_states[EmotionalState.NEUTRAL] = 0.5
...
class NPCManager:
    ...
    def __init__(self, data_file: str = "data/npc_complete.json"):
        ...
        self.last_update = time.time()
        self.economy = None  # Opcjonalna referencja do systemu ekonomii
    ...
    def update(self, delta_time=None, world_context=None):
        ...
        # Dodaj dane ekonomiczne jeśli economy jest dostępne
        if self.economy and "economy" not in world_context:
            world_context["economy"] = {
                "markets": list(self.economy.markets.keys()) if hasattr(self.economy, 'markets') else [],
                "global_events": self.economy.global_events[-5:] if hasattr(self.economy, 'global_events') else []
            }
