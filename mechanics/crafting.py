...
class CraftingSystem:
    ...
    def discover_recipe(self, recipe_id: str, gracz) -> bool:
        ...
        # Sprawdź czy gracz ma atrybut known_recipes
        if hasattr(gracz, 'known_recipes'):
            if recipe_id in gracz.known_recipes:
                return False
        elif recipe_id in self.discovered_recipes:
            return False

        if recipe_id in self.recipes_db:
            # Zapisz do gracza jeśli ma atrybut, w przeciwnym razie do systemu
            if hasattr(gracz, 'known_recipes'):
                gracz.known_recipes.add(recipe_id)
            else:
        ...
        for recipe_id in known_recipe_ids:
            if recipe_id in self.recipes_db:
                ...
                recipe = Recipe(
                    ...
                    skladniki=[
                        RecipeIngredient(
                            przedmiot=ing['przedmiot'],
                            ilosc=ing['ilosc']
                        ) for ing in recipe_data['skladniki']
                    ],
                    wymagane_narzedzia=[
                        RequiredTool(
                            przedmiot=tool.get('przedmiot', tool.get('kategoria', '')),
    ...
    def craft(self, recipe_id: str, materials: List[Dict], crafter_skill: Any = None, player: Any = None) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            player: Obiekt gracza (do sprawdzenia known_recipes)
        ...
        # Sprawdź odkryte receptury - najpierw w graczu, potem w systemie
        known_recipes = set()
        if player and hasattr(player, 'known_recipes'):
            known_recipes = player.known_recipes
        else:
            known_recipes = self.discovered_recipes
