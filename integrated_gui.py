...
"""
    ...
    def execute_command(self, command):
        ...
        # Sprawdź czy jest aktywny dialog z opcjami do wyboru
        if hasattr(self.game_state, 'current_dialogue') and self.game_state.current_dialogue:
            ...
            if dialogue.get('options'):
                # Otwórz interaktywne okno dialogowe z przyciskami (z opóźnieniem żeby menu wyboru NPC się zamknęło)
                self.root.after(100, lambda: self.show_dialogue_window(
                    dialogue.get('npc_name', 'NPC'),
                    "",  # Tekst już wyświetlony przez komendę
                    dialogue['options']
                ))
    ...
    def show_dialogue_window(self, npc_name: str, npc_text: str, options: list):
        """Wyswietla interaktywne okno dialogu z przyciskami opcji."""
        # Zamknij poprzednie okno dialogowe jesli istnieje
        if hasattr(self, 'dialogue_window') and self.dialogue_window:
            try:
        ...
        self.dialogue_window.title(f"Rozmowa z {npc_name}")
        self.dialogue_window.geometry("550x450")
        self.dialogue_window.configure(bg=self.colors['panel'])
        # Wymus pokazanie okna na wierzchu
        self.dialogue_window.transient(self.root)
        self.dialogue_window.grab_set()
        self.dialogue_window.lift()
        self.dialogue_window.focus_force()

        # Ramka na tekst NPC
        text_frame = tk.Frame(self.dialogue_window, bg=self.colors['panel'])
        text_frame.pack(fill=tk.X, padx=15, pady=10)

        tk.Label(text_frame, text=f"{npc_name}:",
                font=self.fonts['header'],
                bg=self.colors['panel'],
                fg=self.colors['accent']).pack(anchor=tk.W)

        # Tekst NPC w scrollowanym polu
        if npc_text:
            npc_text_widget = tk.Text(text_frame, wrap=tk.WORD, height=6,
                                     font=self.fonts['normal'],
                                     bg='#2a2a2a', fg=self.colors['text'],
                                     relief=tk.FLAT, padx=10, pady=10)
            npc_text_widget.insert(tk.END, npc_text)
            npc_text_widget.config(state=tk.DISABLED)
            npc_text_widget.pack(fill=tk.X, pady=5)

        # Separator
        ttk.Separator(self.dialogue_window, orient='horizontal').pack(fill=tk.X, padx=15, pady=5)

        # Ramka na opcje
        options_frame = tk.Frame(self.dialogue_window, bg=self.colors['panel'])
        options_frame.pack(fill=tk.BOTH, expand=True, padx=15, pady=5)

        tk.Label(options_frame, text="Twoje odpowiedzi:",
                font=self.fonts['normal'],
                bg=self.colors['panel'],
                fg=self.colors['text']).pack(anchor=tk.W, pady=(0, 5))

        # Przyciski opcji dialogowych
        if options:
            for i, opt in enumerate(options):
                opt_text = opt.text if hasattr(opt, 'text') else str(opt)
                btn = tk.Button(options_frame,
                              text=f"{i+1}. {opt_text}",
                              width=60, anchor=tk.W,
                              command=lambda idx=i: self.select_dialogue_option(idx),
                              font=self.fonts['normal'],
            ...
            tk.Label(options_frame, text="[Brak dostepnych opcji]",
                    font=self.fonts['normal'],
                    bg=self.colors['panel'],
                    fg='#888888').pack(pady=10)

        # Przycisk zakonczenia rozmowy
        tk.Button(self.dialogue_window,
                 text="Zakoncz rozmowe",
    ...
    def select_dialogue_option(self, choice_index: int):
        """Przetwarza wybor opcji dialogowej i kontynuuje dialog."""
        ...
        # Wyswietl wybrana opcje gracza
        chosen_opt = options[choice_index]
        opt_text = chosen_opt.text if hasattr(chosen_opt, 'text') else str(chosen_opt)
        self.append_text(f"\n[Ty]: {opt_text}\n", 'command')

        # Wywolaj process_choice() z DialogueSystem
        try:
            response, next_text, result, next_options, next_node = \
            ...
            # Wyswietl odpowiedz NPC
            if response:
                self.append_text(f"\n{response}\n", 'normal')
            if next_text:
                self.append_text(f"\n{current['npc_name']}: {next_text}\n", 'story')

            # Sprawdz czy dialog sie konczy
            if result == DialogueResult.END or not next_options:
                ...
                # Odswiez okno dialogowe z nowymi opcjami
                self.show_dialogue_window(current['npc_name'], next_text, next_options)
