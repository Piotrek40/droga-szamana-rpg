...
class PrisonDiseaseQuest(EmergentQuest):
    """Quest: Choroba w celach - losowo, rozprzestrzenia się."""
    
    def __init__(self):
        seed = QuestSeed(
            quest_id="prison_disease",
            name="Zaraza w celach",
            activation_conditions={
                'prison.hygiene': {'operator': '<', 'value': 3},
                'random_chance': {'operator': '>', 'value': 0.4}
            },
            discovery_methods=[
                DiscoveryMethod.WITNESSED,
                DiscoveryMethod.ENVIRONMENTAL,
            ...
            initial_clues={
                'cell_block_a': 'Słychać kaszel i jęki. Ktoś wymiotuje.',
                'prison_infirmary': 'Kolejka chorych więźniów. Felczer wygląda na przeraźonego.',
                'prison_yard': 'Niektórzy więźniowie ledwo stoją na nogach.'
            },
            time_sensitive=True,
            expiry_hours=96,
            priority=9
        )
        super().__init__("prison_disease", seed)
        
        self.discovery_dialogue = {
            'witnessed': [
                "Widzisz jak więzień upada na dziedzińcu. Ma gorączkę i majaczy.",
                "Twój współwięzień wymiotuje kri. To nie wygląda dobrze."
            ],
            'environmental': [
                "Zapach choroby i śmierci unosi się w powietrzu. Coś złego się dzieje.",
                "Coraz więcej cel jest cichych. Zbyt cichych."
            ],
            'told': [
                "Felczer szepcze: 'To zaraza. Jeśli się nie zatrzyma, wymrzemy wszyscy.'",
                "Brutus: 'Szamanie, moi ludzie chorują. Musisz coś zrobić!'"
            ]
        }
    ...
    def _create_branches(self):
        """Tworzy ścieżki rozwiązania questa choroby."""
        
        # ŚCIEŻKA 1: Leczenie szamańskie
        shamanic_branch = QuestBranch(
            "shamanic_healing",
            "Użyj szamańskich rytuałów leczenia"
        )
        shamanic_branch.add_requirement('skill', ('shamanism', 7))
        shamanic_branch.add_requirement('item', 'herbs')
        shamanic_branch.dialogue_options = {
            'preview': "Duchy i zioła mogą powstrzymać zarazę...",
            'resolution': "Odprawiasz rytuały oczyszczenia i leczysz chorych ziołami. Zaraza ustępuje.",
            'healing_ritual': [
                "Zbierasz chore do jednej celi.",
                "Gracz: 'Duchy choroby, słuchajcie mnie!'",
                "",
                "Palisz szałwię i kadzidło.",
                "Intonujesz starożytne zaklęcia.",
                "",
                "Chory 1: 'Czuję... czuję się lepiej!'",
                "Chory 2: 'Gorączka ustępuje!'",
                "",
                "Przygotowujesz napar z ziół:",
                "Gracz: 'Pijcie to trzy razy dziennie. Gorzkie, ale uleczy.'",
                "",
                "Dzień 2:",
                "Felczer: 'To niemożliwe! Oni zdrowieją!'",
                "Gracz: 'Stara wiedza ma swoją moc.'",
                "",
                "Dzień 3:",
                "Brutus: 'Szamanie, jesteś prawdziwym uzdrowicielem!'",
                "Więźniowie: 'Chwała szamanowi! Uratował nas!'",
                "",
                "Naczelnik: 'Nie wiem jak to zrobiłeś, ale dziękuję.'"
            ]
        }
        shamanic_branch.add_consequence('world_state', {
            'prison.disease_cured': True,
            'prison.health': 8,
            'player.reputation_healer': True
        })
        shamanic_branch.add_consequence('relationships', {
            'prisoners': 60,
            'Brutus': 50,
            'warden': 30,
            'medic': 40
        })
        shamanic_branch.add_consequence('delayed', {
            168: {
                'description': 'Więźniowie proszą o regularne leczenie',
                'world_changes': {'player.healer_role': True},
                'npc_reactions': {
                    'prisoners': 'Szamanie, mam ból głowy. Możesz pomóc?'
                }
            }
        })
        self.add_branch(shamanic_branch)
        
        # ŚCIEŻKA 2: Kwarantanna siłowa
        quarantine_branch = QuestBranch(
            "enforce_quarantine",
            "Wymuś kwarantannę chorych"
        )
        quarantine_branch.add_requirement('skill', ('intimidation', 6))
        quarantine_branch.add_requirement('skill', ('leadership', 5))
        quarantine_branch.dialogue_options = {
            'preview': "Trzeba odizolować chorych. Siłą jeśli trzeba.",
            'resolution': "Organizujesz brutalną kwarantannę. Niektórzy umierają, ale zaraza się nie rozprzestrzenia.",
            'quarantine_enforcement': [
                "Gracz do Brutusa: 'Pomóż mi. Musimy oddzielić chorych.'",
                "Brutus: 'Jak?'",
                "Gracz: 'Siłą. Wyrzucimy ich do starego skrzydła.'",
                "",
                "[Segregacja]",
                "Chory: 'Nie! Nie zostawiajcie mnie!'",
                "Gracz: 'Przykro mi. To dla dobra wszystkich.'",
                "Brutus ciągnie chorych do izolatki.",
                "",
                "Więzień: 'To nieludzkie!'",
                "Gracz: 'Wolisz żeby wszyscy umarli?'",
                "",
                "Dzień 3:",
                "W kwarantannie słychać jęki.",
                "Niektórzy już nie żyją.",
                "",
                "Dzień 5:",
                "Zaraza się nie rozprzestrzenia.",
                "Ale 8 więźniów zmarło w izolacji.",
                "",
                "Ocalały z kwarantanny: 'Jesteś potworem szamanie!'",
                "Gracz: 'Zrobiłem co musiałem.'"
            ]
        }
        quarantine_branch.add_consequence('world_state', {
            'prison.disease_contained': True,
            'prison.death_count': 8,
            'prison.fear': 8
        })
        quarantine_branch.add_consequence('relationships', {
            'Brutus': 20,
            'guards': 30,
            'survivors': -40,
            'general_prisoners': -20
        })
        quarantine_branch.add_consequence('delayed', {
            240: {
                'description': 'Rodziny zmarłych chcą zemsty',
                'world_changes': {'vendetta_against_player': True},
                'new_quests': ['family_vendetta_quest']
            }
        })
        self.add_branch(quarantine_branch)
        
        # ŚCIEŻKA 3: Wykradnij lekarstwa
        steal_medicine_branch = QuestBranch(
            "steal_medicine",
            "Wykradnij lekarstwa z infirmerii strażników"
        )
        steal_medicine_branch.add_requirement('skill', ('stealth', 7))
        steal_medicine_branch.add_requirement('skill', ('lockpicking', 5))
        steal_medicine_branch.dialogue_options = {
            'preview': "Strażnicy mają lekarstwa. Muszę je zdobyć.",
            'resolution': "Wykradasz antybiotyki z infirmerii strażników. Ratujesz chorych ale ryzykujesz karę.",
            'heist_scene': [
                "Noc. Przekradasz się do skrzydła strażników.",
                "",
                "*Otwierasz zamek infirmerii*",
                "W środku - szafka pełna lekarstw!",
                "",
                "Bierzesz antybiotyki, środki przeciwgorączkowe, bandaże.",
                "",
                "Głos za tobą: 'Co ty robisz?!'",
                "To strażnik Jenkins!",
                "",
                "Gracz: 'Jenkins, ludzie umierają. Potrzebuję tych leków.'",
                "Jenkins: 'To kradzież! Powinienem cię...'",
                "Gracz: 'Pamiętasz jak oddałem ci klucze? Teraz ja potrzebuję pomocy.'",
                "Jenkins: *waha się* '...Masz 30 sekund. Nie widziałem cię.'",
                "",
                "[Z lekarstwami]",
                "Felczer: 'Skąd to masz?!'",
                "Gracz: 'Nieważne. Ratuj ludzi.'",
                "",
                "Chorzy dostają leki. Większość zdrowieje.",
                "Ale strażnicy wiedzą że ktoś ukradł lekarstwa..."
            ]
        }
        steal_medicine_branch.add_consequence('world_state', {
            'prison.disease_treated': True,
            'prison.medicine_stolen': True,
            'prison.investigation_pending': True
        })
        steal_medicine_branch.add_consequence('relationships', {
            'prisoners': 45,
            'medic': 50,
            'Jenkins': 10,
            'guards': -30
        })
        steal_medicine_branch.add_consequence('delayed', {
            48: {
                'description': 'Śledztwo w sprawie kradzieży',
                'world_changes': {'player.under_investigation': True},
                'npc_reactions': {
                    'guard_captain': 'Znajdę złodzieja i będzie żałował!'
                }
            }
        })
        self.add_branch(steal_medicine_branch)
        
        # ŚCIEŻKA 4: Negocjuj pomoc medyczną
        negotiate_help_branch = QuestBranch(
            "negotiate_medical",
            "Przekonaj naczelnika do sprowadzenia lekarza"
        )
        negotiate_help_branch.add_requirement('skill', ('persuasion', 8))
        negotiate_help_branch.add_requirement('reputation', ('warden', 10))
        negotiate_help_branch.dialogue_options = {
            'preview': "Może uda się przekonać naczelnika...",
            'resolution': "Przekonujesz naczelnika że epidemia zagrozi też strażnikom. Sprowadza lekarza.",
            'negotiation': [
                "Gracz: 'Panie naczelniku, muszę z panem porozmawiać.'",
                "Naczelnik: 'Czego chcesz więźniu?'",
                "Gracz: 'Zaraza się rozprzestrzenia. Niedługo dotrze do strażników.'",
                "Naczelnik: 'Strażnicy są zaszczepieni.'",
                "Gracz: 'Przeciw tej chorobie? Jest nowa, mutuje. Widziałem takie w mojej wiosce.'",
                "Naczelnik: *blednie* 'Mutuje?'",
                "Gracz: 'Tak. Ale jeśli sprowadzimy lekarza teraz, możemy ją zatrzymać.'",
                "Naczelnik: 'Lekarze są drodzy...'",
                "Gracz: 'Tańsi niż pogrzeby strażników. I wyjaśnienia przed zwierzchnikami.'",
                "Naczelnik: '...Dobra. Sprowadzę doktora Millera. Ale ty będziesz odpowiedzialny za porządek.'",
                "",
                "[Przybycie lekarza]",
                "Dr Miller: 'Święty Boże, to dżuma płucna! Szybko, potrzebuję pomocników!'",
                "Gracz pomagając: 'Co mogę zrobić?'",
                "Dr Miller: 'Ty znasz się na medycynie?'",
                "Gracz: 'Trochę. Jestem szamanem.'",
                "Dr Miller: 'Szaman? Cóż, teraz każda pomoc się przyda.'"
            ]
        }
        negotiate_help_branch.add_consequence('world_state', {
            'prison.doctor_arrived': True,
            'prison.disease_treating': True,
            'prison.medical_supplies': 7
        })
        negotiate_help_branch.add_consequence('relationships', {
            'warden': 25,
            'prisoners': 35,
            'doctor': 30
        })
        negotiate_help_branch.add_consequence('delayed', {
            72: {
                'description': 'Doktor proponuje nauczyć cię medycyny',
                'world_changes': {'medical_training_available': True},
                'new_quests': ['medical_training_quest']
            }
        })
        self.add_branch(negotiate_help_branch)
        
        # ŚCIEŻKA 5: Wykorzystaj chaos
        exploit_chaos_branch = QuestBranch(
            "exploit_disease",
            "Wykorzystaj chaos choroby do własnych celów"
        )
        exploit_chaos_branch.add_requirement('skill', ('deception', 7))
        exploit_chaos_branch.dialogue_options = {
            'preview': "Gdy wszyscy są chorzy, nikt nie patrzy...",
            'resolution': "Wykorzystujesz chaos do przejęcia kontroli nad więzieniem. Zostajesz nieoficjalnym władcą.",
            'power_grab': [
                "Udajesz że leczysz, ale wybierasz kogo ratujesz.",
                "",
                "Do Brutusa: 'Mogę cię uleczyć. Ale będziesz mi winien lojalność.'",
                "Brutus: *kaszle krwią* 'Wszystko! Tylko mnie ulecz!'",
                "",
                "Dajesz Brutusowi ukradzione leki.",
                "Brutus: 'Jestem twój szamanie.'",
                "",
                "Do bogatego więźnia: 'Lekarstwo kosztuje. 500 złotych.'",
                "Bogaty: 'Bierz wszystko! Tylko ratuj!'",
                "",
                "Wrogów zostawiasz na śmierć.",
                "Przyjaciele dostają pomoc.",
                "",
                "Po tygodniu:",
                "Kontrolujesz kto żyje, kto umiera.",
                "Wszyscy ci się kłaniają.",
                "",
                "Naczelnik: 'Jak to się stało że ty jeden masz lekarstwa?'",
                "Gracz: 'Duchy mi pomagają. Mogą pomóc i panu... za cenę.'",
                "Naczelnik: 'Jaką cenę?'",
                "Gracz: 'Specjalne traktowanie. I żadnych pytań.'",
                "Naczelnik: '...Zgoda.'"
            ]
        }
        exploit_chaos_branch.add_consequence('world_state', {
            'prison.player_controls': True,
            'prison.death_count': 15,
            'prison.power_structure': 'player_dominated'
        })
        exploit_chaos_branch.add_consequence('relationships', {
            'Brutus': 60,
            'loyal_prisoners': 50,
            'enemies': -100,
            'warden': 20
        })
        exploit_chaos_branch.add_consequence('delayed', {
            336: {  # 2 tygodnie
                'description': 'Więźniowie planują obalić twoją władzę',
                'world_changes': {'coup_planned': True},
                'new_quests': ['prison_coup_quest']
            }
        })
        self.add_branch(exploit_chaos_branch)
        
        # ŚCIEŻKA 6: Stwórz lekarstwo
        create_cure_branch = QuestBranch(
            "create_cure",
            "Stwórz lekarstwo łącząc wiedzę szamańską z medyczną"
        )
        create_cure_branch.add_requirement('skill', ('alchemy', 6))
        create_cure_branch.add_requirement('skill', ('medicine', 4))
        create_cure_branch.dialogue_options = {
            'preview': "Połączę starą wiedzę z nową. Stworzę lekarstwo.",
            'resolution': "Tworzysz skuteczne lekarstwo. Ratujesz wszystkich i zyskujesz sławę.",
            'cure_creation': [
                "W warsztacie więziennym improwizujesz laboratorium.",
                "",
                "Felczer: 'Co robisz?'",
                "Gracz: 'Tworzę lekarstwo. Pomożesz?'",
                "Felczer: 'Szaman robi lekarstwo? To absurd!'",
                "Gracz: 'Zioła mają moc. Trzeba tylko wyekstrahować substancje czynne.'",
                "",
                "Dzień 1: Ekstrakty z wierzby (aspiryna naturalna)",
                "Dzień 2: Wywar z jeżówki (wzmacnia odporność)",
                "Dzień 3: Fermentacja pleśni (prymitywna penicylina)",
                "",
                "Felczer: 'To... to faktycznie może działać!'",
                "Gracz: 'Łączę wiedzę przodków z nauką. Najlepsza kombinacja.'",
                "",
                "Test na chorym:",
                "Chory wypija miksturę.",
                "Po godzinie: gorączka spada!",
                "Po dniu: objawy ustępują!",
                "",
                "Masowa produkcja:",
                "Wszyscy więźniowie pomagają.",
                "W 3 dni zaraza jest pokonana.",
                "",
                "Naczelnik: 'Jak to zrobiłeś?'",
                "Gracz: 'Wiedza szamańska i trochę chemii.'",
                "Naczelnik: 'Jesteś pełen niespodzianek więźniu.'"
            ]
        }
        create_cure_branch.add_consequence('world_state', {
            'prison.disease_cured': True,
            'prison.cure_developed': True,
            'player.reputation_genius': True
        })
        create_cure_branch.add_consequence('relationships', {
            'everyone': 70,
            'medic': 60,
            'warden': 40
        })
        create_cure_branch.add_consequence('delayed', {
            504: {  # 3 tygodnie
                'description': 'Wieści o leku docierają na zewnątrz',
                'world_changes': {'outside_interest': True},
                'new_quests': ['cure_fame_quest'],
                'npc_reactions': {
                    'visiting_noble': 'Słyszałem że więzień stworzył cudowny lek...'
                }
            }
        })
        self.add_branch(create_cure_branch)
