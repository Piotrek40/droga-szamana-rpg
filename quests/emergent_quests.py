...
class PrisonRiotQuest(EmergentQuest):
    """Quest buntu więziennego - powstaje gdy warunki w więzieniu są fatalne"""

    def __init__(self, quest_id: str, seed: QuestSeed):
        super().__init__(quest_id, seed)
        self.riot_intensity: float = 0.0
        self.rioters: List[str] = []
        self.suppressed: bool = False
        self.guards_overwhelmed: bool = False
        ...
        self.discovery_dialogue = {
            "overheard": [
                "Słyszysz szept: 'Już dość! Nadchodzi czas zemsty na strażnikach!'",
                "Więźniowie szepczą: 'Kiedy nadejdzie sygnał, ruszamy!'",
                "Ktoś mruczy: 'Głód nas wykończy... chyba że najpierw my ich wykończymy.'"
            ],
            "witnessed": [
                "Widzisz grupkę więźniów zbierającą prowizoryczną broń.",
                "Zauważasz, że strażnicy są coraz bardziej nerwowi.",
                "Dostrzegasz tajne znaki wymieniane między więźniami."
            ],
            "told": [
                "Brutus szepcze: 'Nadchodzi burza. Wybierz stronę mądrze.'",
                "Zaufany więzień ostrzega: 'Za kilka godzin wszystko się zmieni.'"
            ]
        }

    def _initialize_branches(self):
        """Inicjalizuje gałęzie rozwiązań"""
        # Gałąź 1: Przywództwo buntu
        lead_branch = QuestBranch(
            branch_id="lead_riot",
            description="Poprowadź bunt więźniów przeciwko strażnikom"
        )
        lead_branch.requirements = {
            "reputation_prisoners": 30,
            "allies": 5,
            "charisma": 40
        }
        lead_branch.consequences = {
            "immediate": {
                "prison_control": "prisoners",
                "guards_status": "overwhelmed",
                "player_status": "riot_leader"
            },
            "delayed": [
                {"delay_hours": 24, "effect": "military_response"},
                {"delay_hours": 48, "effect": "siege_begins"}
            ]
        }
        self.add_branch(lead_branch)

        # Gałąź 2: Stłumienie buntu
        suppress_branch = QuestBranch(
            branch_id="suppress_riot",
            description="Pomóż strażnikom stłumić bunt w zamian za przywileje"
        )
        suppress_branch.requirements = {
            "guard_trust": 20,
            "combat_skill": 35
        }
        suppress_branch.consequences = {
            "immediate": {
                "relationships": {"guards": 40, "prisoners": -60},
                "player_status": "collaborator",
                "privileges": ["better_food", "more_freedom"]
            }
        }
        self.add_branch(suppress_branch)

        # Gałąź 3: Ucieczka w chaosie
        escape_branch = QuestBranch(
            branch_id="escape_chaos",
            description="Wykorzystaj chaos buntu do ucieczki"
        )
        escape_branch.requirements = {
            "escape_route_known": True,
            "agility": 30
        }
        escape_branch.consequences = {
            "immediate": {
                "location": "outside_prison",
...
class PrisonMysteryQuest(EmergentQuest):
    """Quest tajemnicy więzienia - odkrywanie mrocznych sekretów"""

    def __init__(self, quest_id: str, seed: QuestSeed):
        super().__init__(quest_id, seed)
        self.clues_found: List[str] = []
        self.mystery_solved: bool = False
        self.secret_revealed: Optional[str] = None

        # Możliwe sekrety do odkrycia
        ...
        self.discovery_dialogue = {
            "found": [
                "Znalazłeś dziwne symbole wyryte na ścianie celi.",
                "Odkryłeś tajny schowek z starymi dokumentami.",
                "Natknąłeś się na mapę z oznaczonymi nieznanymi miejscami."
            ],
            "stumbled": [
                "Przypadkiem natrafiłeś na luźną cegłę kryjącą tajemnicę.",
                "Przewróciłeś się i odkryłeś ukryty właz."
            ],
            "environmental": [
                "W nocy słyszysz dziwne dźwięki dochodzące zza ścian.",
                "Zauważasz, że pewne miejsca są zawsze zamknięte dla więźniów."
            ]
        }
    ...
    def _initialize_branches(self):
        """Inicjalizuje gałęzie rozwiązań"""
        # Gałąź 1: Śledztwo
        investigate_branch = QuestBranch(
            branch_id="investigate",
            description="Dokładnie zbadaj wszystkie poszlaki"
        )
        investigate_branch.requirements = {
            "clues_found": 5,
            "intelligence": 40
        }
        investigate_branch.consequences = {
            "immediate": {
                "knowledge": "prison_secret",
                "reputation": "detective",
                "new_options": True
            }
        }
        self.add_branch(investigate_branch)

        # Gałąź 2: Zignorowanie
        ignore_branch = QuestBranch(
            branch_id="ignore",
            description="Niektóre tajemnice lepiej zostawić w spokoju"
        )
        ignore_branch.requirements = {}
        ignore_branch.consequences = {
            "immediate": {
                "mystery_status": "abandoned",
                "safety": True
            }
        }
        self.add_branch(ignore_branch)

        # Gałąź 3: Wykorzystanie
        exploit_branch = QuestBranch(
            branch_id="exploit",
            description="Wykorzystaj odkrytą tajemnicę dla własnych celów"
        )
        exploit_branch.requirements = {
            "clues_found": 3,
            "cunning": 30
...
class NewPrisonerQuest(EmergentQuest):
    """Quest nowego więźnia - budowanie relacji z nowym więźniem"""

    def __init__(self, quest_id: str, seed: QuestSeed, new_prisoner_id: str = "unknown"):
        super().__init__(quest_id, seed)
        self.new_prisoner_id: str = new_prisoner_id
        self.relationship_level: float = 0.0
        self.trust_established: bool = False
        self.prisoner_background: Optional[str] = None

        # Możliwe tła nowego więźnia
        ...
        self.discovery_dialogue = {
            "witnessed": [
                "Widzisz jak strażnicy prowadzą nowego więźnia.",
                "Zauważasz samotnego nowego więźnia, który wygląda na zdezorientowanego.",
                "Dostrzegasz kogoś nowego siedzącego sam w kącie."
            ],
            "told": [
                "Słyszysz od innych: 'Przyszedł ktoś nowy. Ciekawe, co zrobił.'",
                "Piotr szepcze: 'Ten nowy... ma coś interesującego do ukrycia.'"
            ]
        }

    def _initialize_branches(self):
        """Inicjalizuje gałęzie rozwiązań"""
        # Gałąź 1: Zaprzyjaźnienie się
        befriend_branch = QuestBranch(
            branch_id="befriend",
            description="Nawiąż przyjaźń z nowym więźniem"
        )
        befriend_branch.requirements = {
            "charisma": 25,
            "patience": True
        }
        befriend_branch.consequences = {
            "immediate": {
                "new_ally": True,
                "shared_secrets": True,
                "relationships": {"new_prisoner": 50}
            }
        }
        self.add_branch(befriend_branch)

        # Gałąź 2: Wykorzystanie
        exploit_branch = QuestBranch(
            branch_id="exploit",
            description="Wykorzystaj słabość i dezorientację nowego więźnia"
        )
        exploit_branch.requirements = {
            "manipulation": 30
        }
        ...
        ignore_branch = QuestBranch(
            branch_id="ignore",
            description="Nie angażuj się - niech sam sobie radzi"
        )
        ignore_branch.requirements = {}
        ignore_branch.consequences = {
            "immediate": {
                "no_change": True,
                "potential_enemy": True
            }
        }
        self.add_branch(ignore_branch)
...
def integrate_quests_with_world(quest_engine, world_state: Dict, npc_manager) -> None:
    ...
    for npc in npc_manager.npcs.values():
        if hasattr(npc, 'relationships') and npc.relationships:
            for rel in npc.relationships.values():
                if hasattr(rel, 'get_overall_disposition'):
    ...
    sick_count = sum(
        ...
        if hasattr(npc, 'health') and npc.health < 50
    )
    if sick_count > 2:
    ...
    bribed_guards = sum(
        1 for npc in npc_manager.npcs.values()
        if getattr(npc, 'role', '') == "guard" and
           hasattr(npc, 'semantic_memory') and
           "bribed" in npc.semantic_memory
    )
    if bribed_guards > 0:
        quest_engine.world_state["corruption_level"] = bribed_guards / 10
        quest_engine.world_state["discovered_evidence"] = 1

    # Monitoruj morale więzienia (zabezpieczenie przed dzieleniem przez zero)
    npc_count = len(npc_manager.npcs)
    if npc_count > 0:
        avg_morale = sum(
            getattr(npc, 'emotional_states', {}).get("happy", 0) -
            getattr(npc, 'emotional_states', {}).get("angry", 0)
            for npc in npc_manager.npcs.values()
        ) / npc_count
    else:
        avg_morale = 0.0
    ...
    world_events = getattr(npc_manager, 'world_events', [])
    guard_brutality = sum(
        1 for event in world_events
        if event.get("type") == "guard_violence"
    ) / max(1, len(world_events))

    quest_engine.world_state["guard_brutality"] = guard_brutality

    # Monitoruj głód (zabezpieczenie przed dzieleniem przez zero)
    if npc_count > 0:
        avg_hunger = sum(
            getattr(npc, 'hunger', 0) for npc in npc_manager.npcs.values()
        ) / npc_count
    else:
        avg_hunger = 0.0
...
class QuestIntegrationManager:
    ...
    def __init__(self, quest_engine=None, game_state=None, npc_manager=None):
        ...
        self.quest_factory = {
            ...
            "prison_riot": PrisonRiotQuest,
            "prison_mystery": PrisonMysteryQuest,
            "new_prisoner": NewPrisonerQuest
        }
